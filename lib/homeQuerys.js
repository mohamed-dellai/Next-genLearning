...
export async function getSelectedGifts(user) {
    try {
        const res = await query('select id,name,url,points from public."gifts-chosen" where user_id = $1', [user]);
        return res.rows;
    } catch (error) {
...
export async function insertGiftChosen(user_id,url,points,name){
    try{
        const res=await query('insert into "gifts-chosen"(user_id,url,points,name) values($1,$2,$3,$4) RETURNING id',[user_id,url,points,name])
        return res.rows[0].id
    }
    catch(e){
        console.error("Error inserting gift chosen:", e);
        throw new Error(e)
    }
}
export async function deleteGiftChosen(id){
    try{
        const res=await query('delete from "gifts-chosen" where id=$1',[id])
        return res.rows[0]
    }
    catch(e){
        console.error("Error deleting gift chosen:", e);
        throw new Error(e)
    }
}

export async function deleteRequestedGift(requested_gifts_id) {
    try {
        const res = await query(
            'DELETE FROM public.requested_gifts WHERE requested_gifts_id = $1 RETURNING requested_gifts_id',
            [requested_gifts_id]
        );
        return res.rows[0];
    } catch (error) {
        console.error("Error deleting requested gift:", error);
        throw new Error("Failed to delete requested gift");
    }
}
